...
public class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
	...
	/**
	 * 配置被允许访问此认证服务器的客户端信息 1.内存方式 2. 数据库方式
	 * @param clients
	 * @throws Exception
	 */
	@Override
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		// 内存方式管理客户端信息
		/*
		 * clients.inMemory().withClient("front") // 客户端clientid
		 * .secret("$2a$10$ixV29V933oK5tWZ40Aj0v..pvs.ZeeyOGM7ZIXG21fkNR7uZ78nOG") //
		 * 加密，客户端密码 .resourceIds("quantization-front") // 资源id，针对的是微服务名称，商品管理
		 * .authorizedGrantTypes("authorization_code", "refresh_token") .scopes("all")
		 * // 授权范围标识，哪部分资源可访问（all只是标识，不是说所有资源） .autoApprove(true) // false
		 * 跳到一个授权页面手动点击授权，true不需要手动点授权，直接响应一个授权码
